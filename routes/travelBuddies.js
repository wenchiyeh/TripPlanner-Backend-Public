...
router.put("/tbselect", function (req, res, next) {
  ...
  console.log(tb_id)
  console.log(m_id)
            let sql = "UPDATE memberssignedup SET membersStatus=? WHERE travelBuddies_id=? AND members_id = ?"
...
router.put("/:id", function (req, res, next) {
  ...
  let sql1="UPDATE travelbuddies SET themeName = ?, themePhoto = ?, dateBegin = ?, dateEnd = ?, daysCategory_id = ?, lastApprovedDate = ?, personsNeeded = ?, genderNeeded = ?, estimatedCost = ?, themeIntro = ? WHERE travelbuddies.id=?"
  ...
  let sql4="INSERT INTO categoryRelations SET travelbuddies_id  = ?,regionCategory_id  = ?,cityCategory_id = ? "
  conn.beginTransaction(function(err){
      let tbThemeName=req.body.tbThemeName
      let tbThemePhoto=req.body.tbThemePhoto
      let tbDateBegin=req.body.tbDateBegin
      ...
      conn.query(sql1,[tbThemeName,tbThemePhoto, tbDateBegin, tbDateEnd,tbDaysCategory, tbLastApprovedDate, tbPersonsNeeded,tbGenderNeeded,tbEstimatedCost,tbThemeIntro,id],function(err, result) {
          if (err) { 
            conn.rollback(function() {
              console.log(err)
            });
              ...
              if (err) { 
                ...
                for(let i in tbCityCategory) {
                  ...
                  let query3 = conn.query(sql3, city, function(err3, result3) {
                      ...
                      var query4 = conn.query(sql4, [id ,region,city], function(err, result) {
                            console.log(query3.sql)
                          if (err) { 
                            conn.rollback(function() {
                              console.log(err)
